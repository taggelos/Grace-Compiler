...
public class MethodsVisitor extends GJNoArguDepthFirst<MyType> {
    ...
    */
    public MyType visit(MainClass n) throws Exception {
        ...
        if(n.f14.present()) {   
            for(int i = 0 ; i < n.f14.size() ; i++) {       // Elegxos gia ka8e VarDeclaration
                boolean flag = false;
                if(!st.contains(name).addVar((VariableType)n.f14.nodes.get(i).accept(this)))     // Epanorismos 
                    throw new Exception("Variable " + n.f14.nodes.get(i).accept(this).name + " already exists!");

                String vartype = ((VariableType) n.f14.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) { // Mh egkyros typos
                    for(int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
                    }
                } 
                else 
    ...
    */
    public MyType visit(ClassDeclaration n) throws Exception {
        ...
        n.f4.accept(this);
        if(n.f3.present()) {    
            for(int i = 0; i < n.f3.size(); i++) {              // Elegxos gia ka8e VarDeclaration     
                boolean flag = false;
                if(!st.contains(name).addVar((VariableType)n.f3.nodes.get(i).accept(this)))      // Epanorismos
                    throw new Exception("Variable " + n.f3.nodes.get(i).accept(this).name + " already exists!");

                String vartype = ((VariableType) n.f3.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) {     // Mh egkyros typos
                    for (int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
        ...
        if(n.f4.present()) {    
            for (int i = 0; i < n.f4.size(); i++) {             // Elegxos gia ka8e MethodDeclaration
                boolean flag = false;
                if(st.contains(name).addMethod((MethodType)n.f4.nodes.get(i).accept(this)) == false)    // Epanorismos
                    throw new Exception("Method " + n.f4.nodes.get(i).accept(this).name + " already exists!");

                String vartype = ((MethodType) n.f4.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) {  // Mh egkyros typos
                    for (int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
    ...
    */
    public MyType visit(ClassExtendsDeclaration n) throws Exception {
        ...
        n.f6.accept(this);
        if(n.f5.present()) {     
            for(int i = 0; i < n.f5.size(); i++) {              // Elegxos gia ka8e VarDeclaration
                boolean flag = false;
                if(!st.contains(id).addVar((VariableType)n.f5.nodes.get(i).accept(this)))        // Epanorismos
                    throw new Exception("Variable " + n.f5.nodes.get(i).accept(this).name + " already exists!");

                String vartype = ((VariableType) n.f5.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) {     // Mh egkyros typos
                    for(int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
        ...
        for(int i = 0; i < st.contains(parentid).VarList.size(); i++)           // Pros8etoume tis metablhtes tou Parent Class sto Child Class
            st.contains(id).addVar((VariableType) st.contains(parentid).VarList.get(i));

        if(n.f6.present()) {
            for(int i = 0 ; i < n.f6.size() ; i++) {                    // Elegxos gia ka8e MethodDeclaration
                boolean flag = false;
                if(!st.contains(id).addMethod((MethodType)n.f6.nodes.get(i).accept(this)))               // Epanorismos
                    throw new Exception("Method " + n.f6.nodes.get(i).accept(this).name + " already exists!");
                else if(st.contains(parentid).classContainsMeth(n.f6.nodes.get(i).accept(this).name))  // An yparxei ston Parent Class
                    if(!st.contains(parentid).checkMethod((MethodType)n.f6.nodes.get(i).accept(this)))       
                        throw new Exception("Method " + n.f6.nodes.get(i).accept(this).name + " in extended class has different prototype from the first one!");
                
                String vartype = ((MethodType) n.f6.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) { // Mh egkyros typos
                    for(int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
        ...
        for(int i = 0; i < st.contains(parentid).MethodList.size(); i++)        // Pros8etoume tis sunarthseis tou Parent Class sto Child Class
    ...
    public MyType visit(MethodDeclaration n) throws Exception {
        ...
        if(n.f4.present()) {   
            for(int i = 0; i < m.ParamList.size(); i++) {           
                ...
                String vartype = ((VariableType) m.ParamList.get(i)).type;
        ...
        if(n.f7.present()) {    
            for(int i = 0; i < n.f7.size(); i++) {                  // Elegxos gia ka8e VarDeclaration
                if(!meth.addVar((VariableType) n.f7.nodes.get(i).accept(this)))      // Epanorismos
                    throw new Exception("Variable " + n.f7.nodes.get(i).accept(this).name + " already exists!");

                boolean flag = false;
                String vartype = ((VariableType) n.f7.nodes.get(i).accept(this)).type;
                if(!vartype.equals("int") && !vartype.equals("boolean") && !vartype.equals("int[]")) { // Mh egkyros typos
                    for(int j = 0; j < st.symboltable.size(); j++) {
                        if(vartype.equals(st.symboltable.get(j).name))
                            flag = true;
    ...
    */
    public MyType visit(FormalParameterList n) throws Exception {
        ...
        if(!meth.addParam(par))         // Epanorismos
            throw new Exception("Parameter " + par.name + " already exists!");
        return meth;
    }
    ...
    */
    public MyType visit(FormalParameterTail n) throws Exception {
        ...
        if(n.f0.present()) {                
            for(int i = 0; i < n.f0.size(); i++) {          // Elegxos gia ka8e Parameter
                if(!meth.addParam((VariableType) n.f0.nodes.get(i).accept(this)))        // Epanorismos
                    throw new Exception("Parameter " + n.f0.nodes.get(i).accept(this).name + " already exists!");
            }
        }
        return meth;
    }
