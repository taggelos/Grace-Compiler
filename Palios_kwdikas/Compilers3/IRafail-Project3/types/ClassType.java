...
public class ClassType extends MyType {
    ...
    public void addExtendedMethod(MethodType meth) {
        MethodType newMeth = new MethodType(meth.type, meth.name);
        newMeth.ParamList = meth.ParamList;
        newMeth.VarList = meth.VarList;
        newMeth.from = meth.from;
        newMeth.var_count = meth.var_count;
        newMeth.par_count = meth.par_count;
        newMeth.id = meth.id;
        MethodList.add(newMeth);
    }

    public void addExtendedVar(VariableType var) {
        this.var_count += 1;
        var.id = this.var_count;
        VarList.add(var);
    }
